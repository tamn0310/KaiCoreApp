var RoleController = function () {
    ...
    function registerEvents() {
        ...
        $('#btnSave').on('click', function (e) {
            if ($('#frmMaintainance').valid()) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        kai.notify('Update role successful', 'success');
        ...
        $('body').on('click', '.btn-delete', function (e) {
            e.preventDefault();
            var that = $(this).data('id');
            kai.confirm('Bạn chắc chắn muốn xóa?', function () {
                $.ajax({
                    type: "POST",
                    ...
                    error: function (status) {
                        kai.notify('Có lỗi xảy ra trong quá trình xóa', 'error');
                        kai.stopLoading();
                    }
                });
            });
        ...
        $("#btnSavePermission").off('click').on('click', function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    kai.notify('Lưu quyền thành công', 'success');
                    $('#modal-grantpermission').modal('hide');
                    kai.stopLoading();
                },
                error: function () {
                    kai.notify('Có lỗi xảy ra khi lưu quyền', 'error');
                    kai.stopLoading();
                }
            });
        });
